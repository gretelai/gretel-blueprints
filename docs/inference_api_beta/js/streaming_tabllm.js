export const createStructuredData = async (prompt, rowCallback, num_rows = 10, model_id = 'gretelai/tabular-v0', params) => {
    ...
    let resultCount = 0;
    while (true) {
        const iterateResponse = await _callAPI('v1/inference/tabular/stream/iterate', {
        ...
        if (iterateResult.stream_state.status == 'closed' || resultCount >= num_rows) {
            break;
        }
