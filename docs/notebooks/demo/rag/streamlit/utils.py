...
def render_current_page():
    if st.session_state["current_page"] == "Upload and Chunk":
        ...
        st.session_state["current_page"] == "Generate Synthetic Data"
        and st.session_state["chunked_texts"]
    ):
...
def handle_upload_and_chunk():
    uploaded_files = st.file_uploader(
        "Upload files for text chunking", accept_multiple_files=True, type=["txt", "md"]
    )
    if uploaded_files:
...
def handle_generate_synthetic_data():
    ...
    if gretel_client:
        selected_topics, selected_user_profiles, selected_language = (
        ...
        if st.button("Generate Synthetic Records with Gretel"):
            synthetic_data = generate_synthetic_data(
                ...
                selected_language,
                st.session_state.get("chunked_texts", {}),
            )
...
def process_uploaded_files(
    uploaded_files, chunk_size: int, chunk_overlap: int, min_chunk_chars: int
):
...
def display_chunking_ui():
    ...
    MIN_CHUNK_CHARS = st.number_input(
        ...
        value=1000,
        min_value=1,
        help="Minimum number of characters required for a chunk to be valid.",
    )
...
def show_random_chunk(chunked_texts: dict) -> None:
    ...
    if st.button("Show Random Chunk"):
        ...
        chunk_position_message = (
            f"Showing Chunk {random_index + 1} of {len(all_chunks)}"
        )
...
def construct_prompt(topics, user_profiles, language):
    ...
    SEED_PROMPT_1 = (
        "* `topic`: select a topic from" + ", ".join(topics) + ".\n" if topics else ""
    )
    SEED_PROMPT_2 = (
        "* `user_profile`: The complexity level of the question and truth: chose from"
        + ", ".join(user_profiles)
        + ".\n"
        if user_profiles
        else ""
    ...
    SEED_PROMPT_3 = (
        "* `language`: set the language as" + language + ".\n" if language else ""
    )

    PROMPT = (
        INTRO_PROMPT + SEED_PROMPT_1 + SEED_PROMPT_2 + SEED_PROMPT_3 + COLUMN_DETAILS
    )
    return PROMPT
...
def initialize_navigator(gretel):
    try:
        navigator = gretel.factories.initialize_navigator_api(
            "tabular", backend_model="gretelai-google/gemini-pro"
        )
        return navigator
...
def generate_synthetic_data(
    gretel, selected_topics, selected_user_profiles, selected_language, chunked_texts
):
    ...
    all_chunks = [
        chunk
        for chunks in st.session_state["chunked_texts"].values()
        for chunk in chunks
    ]
...
def display_synthetic_data(synthetic_data: pd.DataFrame) -> None:
    ...
    AgGrid(
        synthetic_data[
            ["topic", "user_profile", "language", "question", "context", "truth"]
        ]
    )
    ...
    if st.button("Evaluate RAG"):
        ...
        try:
            result = subprocess.run(
                ["ai", "chat", "evaluate", "--input-data", jsonl_file_path],
                check=True,
                capture_output=True,
                text=True,
            )
...
def evaluate_rag(uploaded_file=None, data=None):
    ...
    for i, col in enumerate(score_columns):
        axes[i].hist(
            filtered_scores[col].dropna(),
            bins=range(1, 7),
            align="left",
            rwidth=0.8,
            color=blurple_color,
            edgecolor=blurple_color,
        )
        axes[i].set_title(
            f"{col} (Avg: {filtered_scores[col].mean():.2f})", fontsize=font_size + 2
